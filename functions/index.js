...
export const vaultWrite = onRequest({ cors: true }, async (req, res) => {
  ...
  if (op === 'set') {
    if (!docPayload || typeof docPayload !== 'object' || Array.isArray(docPayload)) return res.status(400).json({ error: 'bad_doc' });
    if (Buffer.byteLength(JSON.stringify(docPayload)) > MAX_DOC_BYTES) return res.status(400).json({ error: 'too_large' });
  }
  ...
  let sigOk = false;
  try {
    sigOk = ed25519.verify(b64(signedToken), new TextEncoder().encode(canonicalJson(payload)), b64(publicKey));
  } catch { sigOk = false; }
  ...
  try {
    await db.runTransaction(async (tx) => {
      ...
      if (!registered) tx.set(vaultRef, { writePublicKey: publicKey }, { merge: true }); // first-write-wins
      ...
      if (op === 'delete') {
        ...
        const createdAt = existingApp?.exists ? existingApp.data().createdAt : FieldValue.serverTimestamp();
