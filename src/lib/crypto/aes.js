...
export const importMasterKey = async (jwkData) => {
  return window.crypto.subtle.importKey('jwk', jwkData, { name: 'AES-GCM' }, true, [
    'encrypt',
    'decrypt',
  ]);
};
...
export const argon2DocFields = (params) => ({
  m: params.memorySize,
  t: params.iterations,
...
export const encryptData = async (data, key) => {
  ...
  const encrypted = await window.crypto.subtle.encrypt({ name: ALGO_NAME, iv: iv }, key, encoded);
...
export const decryptData = async (encryptedObj, key) => {
  try {
    ...
    const decrypted = await window.crypto.subtle.decrypt({ name: ALGO_NAME, iv: iv }, key, data);
