...
const LinkDeviceScreen = ({ user, onUnlock, onCancel }) => {
  ...
  return (
    <div className="h-[100dvh] w-full flex flex-col items-center justify-center bg-[#09090b] text-white p-6">
      <div className="bg-[#18181b] p-8 rounded-3xl shadow-2xl max-w-sm w-full border border-[#27272a]">
        <button onClick={onCancel} className="text-xs text-gray-500 hover:text-white flex items-center gap-1.5 mb-5 transition-colors">
          <ArrowLeft size={13} /> Back
        </button>

        {phase === 'received' || phase === 'saving' ? (
          <>
            <div className="mx-auto w-16 h-16 bg-green-500/15 rounded-2xl flex items-center justify-center mb-6">
              <ShieldCheck size={32} className="text-green-400" />
            </div>
            <h2 className="text-2xl font-bold mb-1 text-center tracking-tight">Identity received</h2>
            <p className="text-center mb-6 text-sm text-gray-400">Set a passkey to lock kunji on this device.</p>
            <form onSubmit={handleSave}>
              ...
              <input
                ...
                className="w-full p-4 rounded-xl bg-black border border-[#27272a] text-white mb-3 focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none placeholder-gray-600 font-medium tracking-wide"
              />
          ...
          <>
            ...
            <p className="text-center mb-6 text-sm text-gray-400">
              On a device where kunji is already unlocked, open <strong className="text-gray-200">Security → Scan device QR</strong> and scan this code.
            </p>
