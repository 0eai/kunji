...
const CodeEntryModal = ({ app, onSubmit, onClose }) => {
      ...
      const m =
        e.message === 'invalid_code'
          ...
          : e.message === 'expired_code'
            ? 'That code expired — get a fresh one.'
            : e.message === 'rate_limited'
              ? 'Too many attempts. Wait a minute.'
              : e.message === 'untrusted_callback'
                ? 'Untrusted login request.'
  ...
  return (
    <Sheet onClose={onClose} labelledBy="code-title">
      <h2 id="code-title" className="text-lg font-semibold tracking-tight mb-1">
        Enter login code
      </h2>
